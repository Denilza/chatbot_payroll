...
try:
    ...
    print("✅ Serviços do chatbot inicializados com sucesso")
...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        if chatbot is None:
            raise HTTPException(status_code=503, detail="Serviço do chatbot não disponível")
        ...
        return {"response": response.response, "evidence": [ev.dict() for ev in response.evidence], "sources": response.sources, "conversation_id": response.conversation_id}
...
async def list_employees():
    ...
    try:
        if payroll_data is None:
            raise HTTPException(status_code=503, detail="Dados de folha não disponíveis")
...
async def chatbot_info():
    """Informações sobre o estado do chatbot"""
    return {
...
def find_streamlit_app():
    ...
    for path in possible_paths:
        if os.path.exists(path):
            print(f"✅ Encontrado streamlit_app.py em: {path}")
            return path
    
    print("❌ streamlit_app.py não encontrado nos locais:")
    for path in possible_paths:
        print(f"   - {path}")
    return None
    ...
    try:
        ...
        for port in ports:
            try:
                ...
                if process.poll() is None:  # Ainda está rodando
                    print(f"   ✅ Streamlit iniciado na porta {port}")
                    ...
                    print(f"   ❌ Porta {port} ocupada ou erro")
                    continue
        ...
        if not streamlit_started:
            print("   💡 Dica: Feche outras instâncias do Streamlit ou use:")
...
def main():
    ...
    print("=" * 70)
    print("🤖 CHATBOT FOLHA DE PAGAMENTO - INICIANDO SERVIÇOS")
    print("=" * 70)
    ...
    print("   🤖 Chatbot Info: http://localhost:8000/chatbot/info")
    print("   👥 Employees:    http://localhost:8000/employees")
    print("   📊 Streamlit:    http://localhost:8501 (ou porta alternativa)")
    print("\n🔧 COMANDOS DE TESTE:")
    ...
    print("\n" + "=" * 70)
    print("💡 Use Ctrl+C para parar ambos os serviços")
    print("=" * 70)
